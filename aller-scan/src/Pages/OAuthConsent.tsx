...
export default function OAuthConsent() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // approve/deny. Send the session cookie so the server can list the
        // granted tools for a signed-in user — the same auth the
        // approve/deny call sends; without it the display request is
        // anonymous and shows no tools.
        const res = await fetch(
          `/api/mcp/consent-info?handle=${encodeURIComponent(ctx)}`,
          { credentials: "include" },
        );
        if (!res.ok) {
          setError("This authorization link is invalid or has expired.");
        ...
        if (!data.authenticated) {
          // Rebuild the return path from `ctx` alone — never forward
          // window.location.search raw — so crafted extras on the consent
          // link can't ride through the login round-trip.
          saveReturnTo(window.location.pathname + "?ctx=" + encodeURIComponent(ctx));
  ...
  const respond = async (action: "approve" | "deny") => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/mcp/authorize-grant`, {
        method: "POST",
        credentials: "include",
